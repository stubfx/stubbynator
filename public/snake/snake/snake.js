class Snake {
    ...
    constructor(foodManager, headImg, startingX, startingY) {

        this.startingPosition = createVector(windowWidth * Math.random(),
            (Math.random() > 0.5 ? -100 : windowHeight + 100));
        this.pos = this.startingPosition.copy()
    ...
    move() {
        ...
        let newPos
        if (food) {
            // if the food exists then just go there
            newPos = food
        } else {
            // if not, back to the starting position
            newPos = this.startingPosition
        }
        let xMove = this.pos.x - newPos.x
        let yMove = this.pos.y - newPos.y
